...
export function createLabPanel({ params, onReset, onPreset, onMacro, onModeChange, onPauseChange }) {
    const refreshers = [];
  ...
  `;
    ...
    const state = {
        ...
        sizeVariation: params.sizeVariation.value,
    ...
    refreshers.push(rangeRow(sim, 'sizeVariation [Z/X]', state, 'sizeVariation', 0, 1.5, 0.02, (v) => params.sizeVariation.value = v, () => params.sizeVariation.value));
    ...
    // PRUEBAS DE LABORATORIO (CON RESET)
    const tests = document.createElement('div');
    tests.className = 'group';
    tests.innerHTML = '<h2>Pruebas de comportamiento (Con Reset)</h2>';
    panel.append(tests);
    ...
    // MACROS DE PERFORMANCE (SIN RESET, TRANSICIÓN FLUIDA)
    const macros = document.createElement('div');
    macros.className = 'group';
    macros.innerHTML = '<h2>Macros Performance (Sin Reset)</h2><p>Las fuerzas se moverán solas, conservando la inercia.</p>';
    panel.append(macros);
    for (const [id, label] of [
        ['buildUp', '7 · Tensión Atmosférica'],
        ['drop', '8 · Implosión (Capullo)'],
        ['lotus', '9 · Flor de Loto (Apertura)']
    ]) button(macros, label, () => onMacro(id));
